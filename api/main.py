...
class ApiRequestHandler(http.server.BaseHTTPRequestHandler):
        ...
        if path[0] == "warehouses":         
            ...
            match paths:
                ...
                case 2:
                    ...
                    warehouse = data_provider.fetch_warehouse_pool().get_warehouse(warehouse_id)    #localhost:3000/api/v1/warehouses/1/  return specifiek warehouse id 
                    self.send_response(200)
                    self.send_header("Content-type", "application/json")
                    self.end_headers()
    ...
    def do_PUT(self):
        api_key = self.headers.get("API_KEY")  # Make a header called  API_KEY and the api_key itself
        user = auth_provider.get_user(api_key)
        if user == None:
            self.send_response(401)
            self.end_headers()
            ...
            try:
                path = self.path.split("/")
                if len(path) > 3 and path[1] == "api" and path[2] == "v1":  #add to the url path so path[1] = api and path[2] = v1  -> localhost:3000/api/v1/ {warehouses}
